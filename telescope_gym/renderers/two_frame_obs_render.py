...
class TwoFrameObsRender(Renderer):
    ...
    def render(self, current_state):
        if self.render_mode in self.render_modes:
            ...
            focal_im = ax.imshow(current_state.psf_hist[-2], cmap='inferno', extent=self.ang_extent, norm=self.im_plot_norm) # vmin=log_min, 
            ...
            focal_im = ax.imshow(current_state.psf_hist[-1], cmap='inferno', extent=self.ang_extent, norm=self.im_plot_norm) # vmin=log_min, 
            # plt.title(f'{photon_flux} photons/m^2/s')
            plt.colorbar(focal_im, ax=ax)
